...
void get_high_indx(long,long*,long*,long*,long*,long*,long**,long**,long**);
long find(long*,long);
...
void get_high_indx(long n, long* adj, long* firstOut, long* dfs, long* idfs, long* p, long** high, long** highD, long** highIndx)
{
   (*high) = (long*)malloc(sizeof(long)*n);
   (*highD) = (long*)malloc(sizeof(long)*n);
   (*highIndx) = (long*)malloc(sizeof(long)*firstOut[n]);
   for(long i=0;i<n;i++){(*high)[i]=-1;}
   long* ufparent = (long*)malloc(sizeof(long)*n);
   long* ufsize = (long*)malloc(sizeof(long)*n);
   long* representative = (long*)malloc(sizeof(long)*n);
   for(long i=0;i<n;i++){ufparent[i]=i; ufsize[i]=1; representative[i]=i;}
   char* foundP = (char*)malloc(sizeof(char)*n);
   for(long i=0;i<n;i++){foundP[i]=0;}
   for(long i=n-1;i>=0;i--)
   {
      ...
      for(long j=firstOut[y];j<firstOut[y+1];j++)
      {
         ...
         if(y==p[x]&&!foundP[x]){foundP[x]=1;continue;}
         long v=representative[find(ufparent,x)];
         while(v!=y)
         {
            (*high)[v]=y; (*highD)[v]=x;
            long next=representative[find(ufparent,p[v])];
   ...
   long* Hstack_ = (long*)malloc(sizeof(long)*2*n);
   long* Hfirst_ = (long*)malloc(sizeof(long)*n);
   long* Hnext_ = (long*)malloc(sizeof(long)*2*n);
   long* vertex_ = (long*)malloc(sizeof(long)*2*n);
   for(long i=0;i<n;i++){Hfirst_[i]=-1;}
   long SP=0;
   for(long v=1;v<n;v++)
   {
      long x=(*highD)[v]; long y=(*high)[v];
   ...
   long* Hfirst = (long*)malloc(sizeof(long)*n);
   long* Hnext = (long*)malloc(sizeof(long)*n);
   long* vertex = (long*)malloc(sizeof(long)*n);
   for(long i=0;i<n;i++){Hfirst[i]=-1;}
   SP=0;
   for(long y=n-1;y>=0;y--)
   {
      for(long i=Hfirst_[y];i!=-1;i=Hnext_[i])
      {
         long x=Hstack_[i];
         if(dfs[x]<dfs[y]){continue;}
   ...
   long* adj2 = (long*)malloc(sizeof(long)*firstOut[n]);
   long* currentOut = (long*)malloc(sizeof(long)*n);
   for(long i=0;i<n;i++){currentOut[i]=firstOut[i];}
   long* index = (long*)malloc(sizeof(long)*firstOut[n]);
   for(long x=0;x<n;x++)
   {
      for(long i=firstOut[x];i<firstOut[x+1];i++)
      {
   ...
   long* adj3 = (long*)malloc(sizeof(long)*firstOut[n]);
   for(long i=0;i<n;i++){currentOut[i]=firstOut[i];}
   long* index_ = (long*)malloc(sizeof(long)*firstOut[n]);
   for(long y=0;y<n;y++)
   {
      for(long i=firstOut[y];i<firstOut[y+1];i++)
      {
         long x=adj2[i];
